...
//on récupere dans une variable l'ID searchArticles, qui se trouve sur l'input de recherche dans zoneComunautaire.phtml
let input = document.getElementById("searchArticles");
...
function recupList(){
        ...
        fetch("index.php?route=searchArticles&str="+input.value)
        
            //ensuite la réponse au fetch sur la valeur récuperer dans l'input on le recupere dans la method text()
            .then(response => response.text())
            
            //ensuite le résultat on l'envoie dans la pertie zoneComunautaire.phtml dans la div avec l'id "target"
            .then(resultat => {
